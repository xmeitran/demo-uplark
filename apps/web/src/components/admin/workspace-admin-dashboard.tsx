...
function LarkReminderCardPreview({ slot, localDate, time, recipientLabel }: { slot: WorkspaceReminderSlotCode; localDate: string; time: string; recipientLabel: string }) {
  ...
  return <aside aria-label="Bản xem trước card Lark" className="rounded-2xl border border-border bg-slate-50/70 p-3">
    ...
    <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      ...
      <div className="space-y-3 px-4 py-4 text-xs leading-relaxed text-slate-700"><p>Chào <strong>{recipientLabel}</strong>,</p>{slot === "morning_plan" ? <><p>Kế hoạch Task hôm nay chưa đầy đủ.</p><div className="rounded-lg bg-slate-50 px-3 py-2 text-slate-600">• Bạn chưa lập Task hoặc còn thiếu trường bắt buộc.<br />• Hệ thống sẽ dẫn tới đúng màn hình cần cập nhật.</div></> : null}{slot === "pm_follow_up" ? <><p>Danh sách cần theo dõi sau mốc 08:30:</p><div className="space-y-2 rounded-lg bg-slate-50 px-3 py-2"><div className="flex items-center justify-between gap-2"><span><strong>Nhân sự còn thiếu</strong><br /><span className="text-[11px] text-slate-500">Chưa đủ kế hoạch Task</span></span><span className="flex gap-1"><button type="button" disabled className="rounded-md border border-violet-200 px-2 py-1 text-[10px] font-bold text-violet-700">Gửi nhắc</button><button type="button" disabled className="rounded-md border border-border px-2 py-1 text-[10px] font-bold text-slate-600">Xem</button></span></div></div></> : null}{slot === "evening_actual" ? <><p>Actual Hour ngày hôm nay chưa đầy đủ.</p><div className="rounded-lg bg-slate-50 px-3 py-2 text-slate-600">• Đã ghi: <strong>— / 8 giờ</strong><br />• Task chưa có Actual Hour sẽ được liệt kê tại đây.</div></> : null}<button type="button" disabled className="inline-flex min-h-9 w-full items-center justify-center gap-2 rounded-lg bg-violet-600 px-3 text-xs font-bold text-white opacity-90"><Send className="h-3.5 w-3.5" />{content.action}</button><p className="flex items-start gap-1.5 text-[10px] text-slate-400"><Info className="mt-0.5 h-3 w-3 shrink-0" />Bản xem trước không gửi tin nhắn. Worker sẽ loại trừ Day-off/ngày lễ và ghi nhật ký gửi.</p></div>
    </div>
  </aside>;
}
